...
from q2_types.genome_data import GFF3Format, LociDirectoryFormat
...
def _collect_loci_paths(loci: LociDirectoryFormat) -> list[Path]:
    loci_paths = []

    for relpath, _ in loci.loci.iter_views(GFF3Format):
        loci_paths.append(Path(loci.path) / relpath)

    if not loci_paths:
        raise ValueError("No GFF3 loci files were found in the loci artifact.")

    return loci_paths


def _stage_annotation(loci: LociDirectoryFormat, output_path: Path) -> None:
    loci_paths = _collect_loci_paths(loci)

    if len(loci_paths) == 1:
        shutil.copyfile(loci_paths[0], output_path)
        return

    wrote_header = False
    with output_path.open("w") as out_fh:
        for loci_path in loci_paths:
            with loci_path.open() as in_fh:
                for line in in_fh:
...
def feature_counts(
    ...
    if strand_mode not in _FEATURECOUNTS_STRAND_MODE:
        raise ValueError(
            "strand_mode must be one of: "
            f"{', '.join(sorted(_FEATURECOUNTS_STRAND_MODE))}."
        )
    ...
    with tempfile.TemporaryDirectory(prefix="q2-subread-") as tmpdir:
        ...
        annotation_path = tmpdir_path / "annotation.gff3"
        output_path = tmpdir_path / "feature-counts.tsv"

        _stage_annotation(loci, annotation_path)
...
def align_reads(
    ...
    sequence_paths = [Path(reads.path) / relpath for relpath, _ in reads.sequences.iter_views(FastqGzFormat)]
    has_reverse_reads = any(
        _parse_casava_fastq_filename(path)[1] == "R2"
    ...
    if has_reverse_reads:
        ...
        read_lookup = {sample_id: (forward_path, None) for sample_id, forward_path in collected_reads}
